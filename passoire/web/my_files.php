...
$stmt = $conn->prepare("SELECT f.id, f.path, f.type, f.date, f.ownerid, l.hash 
                        FROM files f 
                        JOIN links l ON f.id = l.fileid 
                    ...
                    <?php if ($files): ?>
                        <table class="w3-table">
                            ...
                            <?php foreach ($files as $file): ?>
                                <tr>
                                    <td><a href="link.php?file=<?= htmlspecialchars($file['hash']) ?>"><?= htmlspecialchars(basename($file['path'])) ?></a></td>
                                    <td><?= htmlspecialchars($file['type']) ?></td>
                                    <td><?= htmlspecialchars($file['date']) ?></td>
                                    <td>
                                        <!-- Copy Link Button -->
                                        <button class="w3-button w3-theme w3-padding" onclick="copyLink('<?= htmlspecialchars($file['hash']) ?>')">Copy Link</button>
